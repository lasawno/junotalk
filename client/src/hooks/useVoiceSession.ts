 ...
 *
 * Example:
 *   const { phase, start, stop, speak, unlock, audioLevel } = useVoiceSession({
 *     lang: fromLang,
...
export interface UseVoiceSessionOptions
  extends Omit<VoiceSessionOptions, 'onPhaseChange' | 'onAudioLevel'> {
  /** When true, enable barge-in (cut TTS when user starts speaking). */
  bargeIn?: boolean;
  /** When true, use Whisper STT instead of browser SpeechRecognition. */
  whisperMode?: boolean;
}
...
export interface UseVoiceSessionResult {
  ...
  isSpeaking: boolean;
  /** Live microphone signal level, 0–100. Useful for animating the orb. */
  audioLevel: number;
  start: () => void;
  stop: () => void;
  speak: (text: string, ttsLang?: string, ttsVoiceId?: string) => Promise<void>;
...
export function useVoiceSession(opts: UseVoiceSessionOptions): UseVoiceSessionResult {
  ...
  useEffect(() => {
    const session = new VoiceSession({
      ...
      bargeIn:      optsRef.current.bargeIn,
      whisperMode:  optsRef.current.whisperMode,
  ...
  // Sync mutable options to the session without recreating it
  useEffect(() => { sessionRef.current?.setLang(opts.lang); }, [opts.lang]);
  useEffect(() => { if (opts.voiceId)    sessionRef.current?.setVoiceId(opts.voiceId); }, [opts.voiceId]);
  useEffect(() => { if (opts.speechSpeed != null) sessionRef.current?.setSpeed(opts.speechSpeed); }, [opts.speechSpeed]);
